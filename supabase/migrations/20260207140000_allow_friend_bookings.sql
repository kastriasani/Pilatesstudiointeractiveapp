-- Allow same user to book the same slot multiple times (for friend invitations)
-- Subsequent bookings by the same user at the same slot are marked as friend bookings

-- Add is_friend_booking column to reservations table
ALTER TABLE reservations ADD COLUMN IF NOT EXISTS is_friend_booking BOOLEAN DEFAULT FALSE;

-- Update the create_reservation RPC to allow duplicates and mark as friend bookings
