-- Fix: allow bookings on pending (renewed) packages before admin activation
-- Previously, create_reservation required package_status = 'active' for all
-- non-first-session bookings, blocking renewed unpaid packages entirely.
-- Now pending packages are also allowed (up to 2 sessions enforced in app layer).
-- Reservations on pending packages get status 'pending' (not 'confirmed').
