...
export function IndividualTraining({ onBack, language, onLogoClick }: IndividualTrainingProps) {
    ...
    try {
      ...
      for (let i = 0; i < liveDays.length; i++) {
        ...
        const availableTimeSlots: TimeSlotInfo[] = apiSlots.map((slot: any) => {
          ...
          // Only individual class_type slots are bookable for individual packages
          const isBookable = classType === 'individual' && available > 0 && !isPastTime;
        ...
        // Only add dates that have at least one bookable individual slot
  ...
  const handleSubmit = async (packageType: '1class' | '8classes' | '12classes') => {
    ...
    try {
      const packageTypeMap: Record<string, string> = {
        '1class': 'individual1',
        '8classes': 'individual8',
        '12classes': 'individual12',
      };
  ...
  const getSessionLabel = (pkgType: string) => {
    if (pkgType === 'individual1') return '1';
    if (pkgType === 'individual8') return '8';
    if (pkgType === 'individual12') return '12';
    return '';
  };
    ...
    <div className="h-full overflow-y-auto px-5 py-4 pt-12 relative bg-gradient-to-br from-[#faf9f7] via-[#f5f3f0] to-[#f0ede8]">
          ...
          <div className="bg-white/95 backdrop-blur-md rounded-3xl p-7 max-w-sm w-full shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] border border-white/20 animate-scale-in">
            ...
            <div className="bg-gradient-to-br from-[#f5f0ed] to-[#f0ebe6] rounded-xl p-4 mb-5">
              ...
              <p className="text-sm text-[#3d2f28] font-medium pl-6">
                {getSessionLabel(packageData.packageType)} {t.sessions || 'CLASSES'} ({t.individualTrainingPageTitle || 'Individual'})
              </p>
            </div>
          ...
          <div className="bg-white/95 backdrop-blur-md rounded-3xl p-7 max-w-sm w-full shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] border border-white/20 animate-scale-in">
            ...
            {successData?.skippedFirstSession && (
              <div className="mb-4 bg-gradient-to-br from-[#f5f0ed] to-[#f0ebe6] rounded-xl p-4 space-y-2">
                ...
                <p className="text-sm text-[#3d2f28] font-medium pl-6">
                  {getSessionLabel(successData.packageType)} {t.sessions || 'CLASSES'} ({t.individualTrainingPageTitle || 'Individual'})
                </p>
              </div>
            )}
                      ...
                      {bookingSlots.find(slot => slot.dateKey === expandedDate)!.timeSlots.map((timeSlot) => (
                        ...
                        >
                          ...
                          <div className="text-[10px] mt-0.5 opacity-70">
                            {timeSlot.classType !== 'individual'
                              ? (timeSlot.classType === 'duo' ? 'Duo' : (t.groupClass || 'Group'))
                              : timeSlot.isBookable
      ...
      <div className="flex items-center mb-6">
        ...
        <div className="flex-1">
          <h1 className="text-lg font-semibold text-[#3d2f28] tracking-tight">{t.individualTrainingPageTitle}</h1>
      ...
      <div className="bg-gradient-to-br from-white via-white to-[#f8f9f4] rounded-3xl p-5 shadow-[0_4px_20px_rgba(0,0,0,0.06)] mb-6 border border-[#e8e6e3]/50">
        <h2 className="text-sm font-semibold text-[#3d2f28] mb-4 tracking-tight">{t.whatIsIndividual}</h2>
        <div className="space-y-3">
          <div className="flex items-start gap-3">
            ...
            <p className="text-xs text-[#6b5949] leading-relaxed font-medium">{t.individualBenefit1}</p>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-5 h-5 rounded-full bg-gradient-to-br from-[#9ca571] to-[#8a9463] flex items-center justify-center flex-shrink-0 mt-0.5 shadow-sm">
              <Check className="w-3 h-3 text-white" />
            </div>
            <p className="text-xs text-[#6b5949] leading-relaxed font-medium">{t.individualBenefit2}</p>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-5 h-5 rounded-full bg-gradient-to-br from-[#9ca571] to-[#8a9463] flex items-center justify-center flex-shrink-0 mt-0.5 shadow-sm">
              <Check className="w-3 h-3 text-white" />
            </div>
            <p className="text-xs text-[#6b5949] leading-relaxed font-medium">{t.individualBenefit3}</p>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-5 h-5 rounded-full bg-gradient-to-br from-[#9ca571] to-[#8a9463] flex items-center justify-center flex-shrink-0 mt-0.5 shadow-sm">
              <Check className="w-3 h-3 text-white" />
            </div>
            <p className="text-xs text-[#6b5949] leading-relaxed font-medium">{t.individualBenefit4}</p>
          </div>
        </div>
      </div>
